...
class Value {
  constructor(node, data = node instanceof ArrayNode ? [] : {}) {
    ...
    this.onSet = utils.createEvent() // When data is updated (reference equality)

    this.onChange = utils.createEvent() // When a Value becomes referenced

    this.onReference = utils.createEvent() // When a Value becomes de-refenerced

    this.onUnreference = utils.createEvent()
    this.data = data
...
const deepReference = rootValue => {
  ...
  const onReference = utils.createEvent()
  const onUnreference = utils.createEvent()
...
class Selection {
  constructor(node) {
    ...
    this.onSelect = utils.createEvent()
    /**
     * Emitted when a child selection is unselected
     */

    this.onUnselect = utils.createEvent()
  }

  add(selection, isKeySelection = false) {
    !(selection !== this)
      ? utils.invariant(false, `Circular selections are not permitted!`)
      : void 0
...
const buildFragmentTree = ({ SPACE, hug, indent, formatter }, tree) => {
  ...
  if (formatter.options.prettify && tree.selection.name) {
...
tslib.__decorate([utils.computed], SelectionTree.prototype, 'path', null)

tslib.__decorate([utils.computed], SelectionTree.prototype, 'alias', null)

tslib.__decorate([utils.computed], SelectionTree.prototype, 'key', null)
...
class Formatter {
  constructor({
    prettify = 'development' !== 'production',
    variables = false,
    fragments = 'inline',
  } = {}) {
    this.formatter = this
...
class NodeEntry {
  ...
  match(data) {
    !(this.node instanceof Matchable)
      ? utils.invariant(false, `${this.node} does not support pattern matching`)
      : void 0
...
class Cache extends Disposable {
  constructor(node) {
    ...
    this.onRootValueChange = utils.createEvent()
...
class Commit extends Disposable {
  constructor(plugins, stack, fetchAccessors) {
    ...
    this.onActive = utils.createEvent()
    this.onIdle = utils.createEvent()
    this.onFetched = utils.createEvent()
  ...
  stage(accessor, ...queries) {
    ...
    if (this.disposed || accessor.status !== exports.NetworkStatus.idle)
      return unstage
  ...
  unstage(accessor) {
    ...
    if (this.accessors.has(accessor)) {
      accessor.status = exports.NetworkStatus.idle
    }
...
class Scheduler extends Disposable {
  constructor(fetchAccessors, plugins = new Plugins(), interval = 50 + 100) {
    super()
    this.fetchAccessors = fetchAccessors
    this.plugins = plugins
...
const onDataChange = accessor => {
  const onDataChange = utils.createEvent()
  let dispose
...
class Interceptor {
  constructor() {
    ...
    this.onAccessor = utils.createEvent()
  }
...
class Accessor extends Disposable {
  constructor(parent, selection, node = selection.node) {
    ...
    this._resolved = true
    this.onValueChange = utils.createEvent() // Equality check only

    this.onDataChange = onDataChange(this)
    this.onResolvedChange = utils.createEvent()
    this.onStatusChange = utils.createEvent()
    this.onInitializeExtensions = utils.createEvent()
...
tslib.__decorate([utils.computed], Accessor.prototype, 'selectionPath', null)

tslib.__decorate([utils.computed], Accessor.prototype, 'path', null)
...
class FieldsNode {
  constructor(fields, { name }) {
    this.name = name
    this.fields = utils.lazyGetters(fields, (fieldName, field) => {
...
class Extension {
  ...
  childIndex() {
    return memo.childIndex(() => {
      ...
      !(this.node instanceof ArrayNode) ? utils.invariant(false) : void 0
  ...
  childField(field) {
    return memo.childField(() => {
      ...
      !(this.node instanceof FieldsNode) ? utils.invariant(false) : void 0
...
tslib.__decorate([utils.computed], Extension.prototype, 'fragmentKey', null)

tslib.__decorate([utils.computed], Extension.prototype, 'fragment', null)

tslib.__decorate([utils.computed], Extension.prototype, 'path', null)
...
tslib.__decorate([utils.computed], ComputableExtension.prototype, 'data', null)
...
tslib.__decorate([utils.computed], ComputedExtension.prototype, 'data', null)
...
tslib.__decorate([utils.computed], NodeContainer.prototype, 'innerNode', null)
...
const getAbstractImplementation = (node, typename) => {
  if (node instanceof Abstract && typename) {
    ...
    !implementation
      ? utils.invariant(
          false,
          `'${typename}' is not a valid subtype of ${node}`
        )
      : void 0
...
const memo$1 =
  /*#__PURE__*/
  utils.createMemo()
class ArrayNode extends mixClasses.Mix(
  mixClasses.Generic(NodeContainer),
  Matchable
) {
...
class InterfaceNode extends mixClasses.Mix(
  FieldsNode,
  mixClasses.Generic(Abstract)
) {
  ...
  toString() {
    return mixClasses.getMixin(this, FieldsNode).toString()
  }
}
...
const TYPENAME_NODE =
  /*#__PURE__*/
  new ScalarNode()
class ObjectNode extends mixClasses.Mix(FieldsNode, Matchable) {
...
class InputNode {
  constructor(inputs, { name }) {
    this.name = name
    this.inputs = utils.lazyGetters(inputs)
  }
...
class Arguments {
  constructor(inputs, required = false) {
    ...
    this.inputs = utils.lazyGetters(inputs, (fieldName, field) => {
      // Called when the getter prop is evaluated
      field.name = fieldName
    })
  }
...
const preload = (_func, ..._args) => {
  ...
  utils.invariant(false, `babel-plugin-gqless is required for preloading`)
}
...
function resolved(data, options) {
  const isResolved = accessor =>
    ...
    false
      ? accessor.status === exports.NetworkStatus.idle
      : accessor.status !== exports.NetworkStatus.loading
...
exports.ArgumentsField = ArgumentsField
exports.ArrayNode = ArrayNode
exports.Cache = Cache
exports.Client = Client
exports.ComputableExtension = ComputableExtension
...
exports.FragmentAccessor = FragmentAccessor
exports.GET_KEY = GET_KEY
exports.INDEX = INDEX
exports.IndexAccessor = IndexAccessor
exports.InputNode = InputNode
exports.InputNodeField = InputNodeField
...
exports.RootAccessor = RootAccessor
...
exports.accessorInterceptors = accessorInterceptors
exports.afterTransaction = afterTransaction
exports.buildArguments = buildArguments
exports.buildQuery = buildQuery
exports.buildSelectionTree = buildSelectionTree
exports.buildSelections = buildSelections
exports.createExtension = createExtension
exports.createValue = createValue
exports.fragmentOn = fragmentOn
exports.getAbstractImplementation = getAbstractImplementation
exports.getAccessor = getAccessor
exports.getExtensions = getExtensions
exports.getSelection = getSelection
exports.getValue = getValue
exports.interceptAccessor = interceptAccessor
exports.keyIsEqual = keyIsEqual
exports.keyIsValid = keyIsValid
...
//# sourceMappingURL=gqless.cjs.development.js.map
