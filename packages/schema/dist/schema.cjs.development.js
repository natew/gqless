...
class SchemaFile extends File {
  ...
  generateFieldGetter(field) {
    this.import(CORE, gqless.FieldNode.name)
    return `get ${field.name}() {
      return new ${gqless.FieldNode.name}(${this.generateType(
      field.type
  ...
  generateNode(type) {
    switch (type.kind) {
      case 'OBJECT': {
        this.import(CORE, gqless.ObjectNode.name)
        return `new ${gqless.ObjectNode.name}({
      ...
      case 'INTERFACE': {
        this.import(CORE, gqless.InterfaceNode.name)
        return `new ${gqless.InterfaceNode.name}({
          ${Object.values(type.fields)
            .map(field => this.generateFieldGetter(field))
            .join(',')}
        },
      ...
      case 'UNION': {
        this.import(CORE, gqless.UnionNode.name)
        return `new ${gqless.UnionNode.name}([${type.possibleTypes.map(type =>
          this.getNode(type)
        )}])`
      }

      case 'SCALAR': {
        this.import(CORE, gqless.ScalarNode.name)
        return `new ${gqless.ScalarNode.name}({ name: ${JSON.stringify(
          type.name
        )}, extension: ${this.getExtension(type.name)} })`
      }

      case 'INPUT_OBJECT': {
        this.import(CORE, gqless.InputNode.name)
        return `new ${gqless.InputNode.name}({
          ${Object.values(type.inputFields)
            .map(field => {
              this.import(CORE, gqless.InputNodeField.name)
              return `get ${field.name}() {
                return new ${gqless.InputNodeField.name}(${this.generateType(
                field.type
              )}, ${field.type.nullable})
              }`
            })
      ...
      case 'ENUM': {
        this.import(CORE, gqless.EnumNode.name)
        return `new ${gqless.EnumNode.name}({ name: ${JSON.stringify(
          type.name
        )} })`
      }
    }
  ...
  generateType(type) {
    this.import(CORE, gqless.ArrayNode.name)

    if (type.kind === 'LIST') {
      return `new ${gqless.ArrayNode.name}(${this.generateType(type.ofType)}, ${
        type.nullable
      })`
    }
  ...
  generateArguments(args) {
    ...
    this.import(CORE, gqless.Arguments.name)
    const argsRequired = !Object.values(args).find(arg => arg.nullable)
      ? ', true'
      : ''
    return `new ${gqless.Arguments.name}({
      ${Object.entries(args)
        .map(([name, type]) => {
          this.import(CORE, gqless.ArgumentsField.name)
          return `get ${name}() {
            return new ${gqless.ArgumentsField.name}(${this.generateType(
            type
          )}, ${type.nullable})}`
        })
        .join(',')}
